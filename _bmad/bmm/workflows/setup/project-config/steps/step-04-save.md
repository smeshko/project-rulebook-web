...
- 💾 Save to `_bmad/project-config.yaml` only after confirmation
...
2. Ensure `_bmad/` directory exists
3. Write configuration to `{configFile}`
4. Confirm: "Configuration saved successfully to `_bmad/project-config.yaml`"
...
Your configuration has been saved to `_bmad/project-config.yaml`.

**Next Steps:**
You can now use the following workflows with your configured settings:
- `/test` - Run all configured test commands
- `/document` - Verify and update documentation
